...
이 레포는 **Clawket(LLM-네이티브 작업관리 + 로컬 RAG)의 SSOT 데이터(콘텐츠)** 입니다. 노드 `.md`를 수정해 PR을 올리면, CI가 무결성을 검사한 뒤 머지합니다. 본문은 플러그인·Rust CLI/데몬·웹/데스크탑 코드에서 역추론한 자연어 문서로, 현재 confidence 대부분 `inferred` — **담당자 검증으로 `high` 로 승급**하는 것이 기여의 핵심입니다. 도구는 별도 레포입니다 — 작성 스킬·MCP 는 [`ssot-studio/ssot-plugin`](https://github.com/ssot-studio/ssot-plugin), 뷰어(웹)는 [`ssot-studio/ssot-web`](https://github.com/ssot-studio/ssot-web).
...
node $SSOT/verify.mjs   ./ssot 90 --root <clawket-코드-레포-루트>
